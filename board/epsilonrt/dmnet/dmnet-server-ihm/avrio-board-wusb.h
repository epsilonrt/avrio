 ...
 */
#define WUSBIO_CTL_DDR  SPI_DDR
#define WUSBIO_CTL_PORT SPI_PORT
#define WUSBIO_RESET    _BV(0) /* Reset: Actif à 0 */
#define WUSBIO_PD       _BV(3) /* Power Down: Actif à 0 */

/* 
 * Signal IRQ
 */
#define WUSBIO_IRQ_DDR  SPI_DDR
#define WUSBIO_IRQ_PORT SPI_PORT
...
vWusbIoInit (void)) {
  ...
  MCUCSR &= ~_BV(ISC2); // déclenche sur front descendant
}
...
vWusbIoIrqEnable (void)) {
  ...
  sbi (GIFR, INTF2); /* clear le flag d'it */
  sbi (GICR, INT2);
}
