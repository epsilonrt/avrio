 ...
 */
#ifndef _BERTOS_FIFOBUF_H_
#define _BERTOS_FIFOBUF_H_
 ...
 *
 * @note Calling fifo_isempty() is safe while a concurrent
 ...
 */
INLINE bool
fifo_isempty(const xFifoBuffer *fb) {
 ...
 *
 * @note Calling fifo_isfull() is safe while a concurrent
 ...
 */
INLINE bool
fifo_isfull (const xFifoBuffer *fb) {
 ...
 *
 * @note Calling \c fifo_push() on a full buffer is undefined.
 *       The caller must make sure the buffer has at least
 *       one free slot before calling this function.
 *
 * @note It is safe to call fifo_pop() and fifo_push() from
 ...
 */
INLINE void
fifo_push(xFifoBuffer *fb, uint8_t c) {
 ...
 *
 * @note Calling \c fifo_pop() on an empty buffer is undefined.
 ...
 *
 * @note This is actually only needed for 8-bit processors.
 *
 * @sa fifo_isempty()
 */
INLINE bool
fifo_isempty_locked (const xFifoBuffer *fb) {
  bool result;
 ...
 */
INLINE void
fifo_push_locked(xFifoBuffer *fb, uint8_t c) {
...
INLINE uint8_t
fifo_pop_locked(xFifoBuffer *fb) {
  uint8_t c;
 ...
 */
INLINE void
fifo_flush_locked (xFifoBuffer *fb) {
 ...
 */
INLINE bool
fifo_isfull_locked (const xFifoBuffer *_fb) {
  bool result;
 ...
 */
INLINE void
fifo_init (xFifoBuffer *fb, uint8_t *buf, size_t size) {
...
/*
 * @return Lenght of the xFifoBuffer \a fb.
 */
INLINE size_t
fifo_len (xFifoBuffer *fb) {
