 ...
 * 
 * Uses providers/consumer model.  When an interrupt drives the
 * consumer/provider side use the interrupt calls
 * 
...
int16_t
AvrXPutFifo (pAvrXFifo p, uint8_t c) {
  uint8_t t = p->in + 1;
  ...
  p->buf[p->in] = c;
  p->in = t;
  vAvrXSetSemaphore (&p->Producer);
  return FIFO_OK;
}

int16_t
AvrXPullFifo (pAvrXFifo p) {
  ...
  if (t >= p->size)
    t = 0;
  p->out = t;
  vAvrXSetSemaphore (&p->Consumer);
  return c;
}

void
AvrXWaitPutFifo (pAvrXFifo p, uint8_t c) {
  while (AvrXPutFifo (p, c) == FIFO_ERR)
    vAvrXWaitSemaphore (&p->Consumer);
}

int16_t
AvrXWaitPullFifo (pAvrXFifo p) {
  int16_t c;

  while ((c = AvrXPullFifo (p)) == FIFO_ERR)
    vAvrXWaitSemaphore (&p->Producer);
  return c;
}

void
AvrXFlushFifo (pAvrXFifo p) {
  p->in = 0;
  p->out = 0;
  vAvrXResetSemaphore (&p->Consumer);
  vAvrXResetSemaphore (&p->Producer);
}

// Return next character to be removed (without removing it)

int16_t
AvrXPeekFifo (pAvrXFifo p) {
  if (p->in == p->out)
    return FIFO_ERR;
  else
    return p->buf[p->out];
