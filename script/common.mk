...
rebuild: begin gccversion sizebefore clean_list build sizeafter end
...
sizebefore:
	@if test -f $(TARGET).elf; then echo; echo $(MSG_SIZE_BEFORE); $(ELFSIZE); 2>/dev/null; fi

sizeafter:
	@if test -f $(TARGET).elf; then echo; echo $(MSG_SIZE_AFTER); $(ELFSIZE); 2>/dev/null; fi
...
clean_list :
	...
	$(REMOVEDIR) $(DESTDIR)
ifeq ($(AVRX),ON)
#	@$(MAKE) BOARD=$(BOARD) OPT=$(OPT) MCU=$(MCU) -C $(AVRIOSRCDIR)/avrx cleanlib
endif
