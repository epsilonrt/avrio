...
#include <string.h>
#include <fatfs/diskio.h>		/* Common include file for FatFs and disk I/O layer */
...
#define	CF_SOCKINS		(!(PINC & 0x03))	/* Card detected. yes:true, no:false, default:true */
...
#define MM_SOCKWP	(PINB & 0x20)		/* Write protected. yes:true, no:false, default:false */
#define MM_SOCKINS	(!(PINB & 0x10))	/* Card detected.   yes:true, no:false, default:true */
...
{
	CS_HIGH();
	MM_rcvr_spi();	/* Dummy clock (force DO hi-z for multiple slave SPI) */
}
...
{
	CS_LOW();
	MM_rcvr_spi();	/* Dummy clock (force DO enabled) */

	if (MM_wait_ready()) return 1;	/* OK */
	MM_deselect();
	return 0;	/* Timeout */
}
...
void disk_timerproc (void)
{
	...
	if (CF_SOCKINS) {			/* CF inserted */
		Stat[0] &= ~STA_NODISK;
	} else {					/* CF socket empty */
	...
	if (MM_SOCKWP)		/* MMC write protected */
		s |= STA_PROTECT;
	else				/* MMC not write enabled */
		s &= ~STA_PROTECT;

	if (MM_SOCKINS)		/* MMC inserted */
		s &= ~STA_NODISK;
